...
use numbers::{
    Abs, BooleanType, FixedPointNumber, FromWithContext, Gen1DArray, IsGreater, Max, Min,
};
...
impl<
        ...
        C,
        const N: usize,
    > ActivationFn<FixedPointNumber<SIZE, PRECISION, T, C>, C, N>
    ...
    FixedPointNumber<SIZE, PRECISION, T, C>: FromWithContext<f32, C>,
{
    fn activate_multiple(
        ...
        let mut new_v: Gen1DArray<FixedPointNumber<CALC_SIZE, CALC_PRECISION, T, C>, C, N> =
        ...
        let r1: Gen1DArray<FixedPointNumber<CALC_SIZE, CALC_PRECISION, T, C>, C, N> = new_v * &b1;
        let r2: Gen1DArray<FixedPointNumber<CALC_SIZE, CALC_PRECISION, T, C>, C, N> = v3 * &b2;
        let r3: Gen1DArray<FixedPointNumber<CALC_SIZE, CALC_PRECISION, T, C>, C, N> = v5 * &b3;
        let r4: Gen1DArray<FixedPointNumber<CALC_SIZE, CALC_PRECISION, T, C>, C, N> = v7 * &b4;

        let res: Gen1DArray<FixedPointNumber<CALC_SIZE, CALC_PRECISION, T, C>, C, N> =
            ((r0 + &r1) + &(r2 + &r3)) + &r4;
...
impl<const SIZE: usize, const PRECISION: usize, T: BooleanType<C>, C, const N: usize>
    ActivationFn<FixedPointNumber<SIZE, PRECISION, T, C>, C, N> for SigmoidMyersAproxAF
where
    FixedPointNumber<SIZE, PRECISION, T, C>: FromWithContext<f32, C>,
{
    fn activate(
        ...
        let v2: FixedPointNumber<SIZE, PRECISION, T, C> = (&v >> 6) + &b1;
        let v3: FixedPointNumber<SIZE, PRECISION, T, C> = (&v >> 5) + &b2;
        let v4: FixedPointNumber<SIZE, PRECISION, T, C> = (&v >> 3) + &b3;
        let v5: FixedPointNumber<SIZE, PRECISION, T, C> = (&v >> 2) + &b4;
        let v6: FixedPointNumber<SIZE, PRECISION, T, C> = (&v >> 3) + &b5;
        let v7: FixedPointNumber<SIZE, PRECISION, T, C> = (&v >> 5) + &b6;
        let v8: FixedPointNumber<SIZE, PRECISION, T, C> = (&v >> 6) + &b7;
    ...
    fn activate_and_derivative(
        ...
        */
        ...
        let res = comp1.mux_number_ref(v8, &v9);
        let res = comp2.mux_number_ref(v7, &res);
        let res = comp3.mux_number_ref(v6, &res);
        let res = comp4.mux_number_ref(v5, &res);
        let res = comp5.mux_number_ref(v4, &res);
        let res = comp6.mux_number_ref(v3, &res);
        let res = comp7.mux_number_ref(v2, &res);
        let res = comp8.mux_number_ref(v1, &res);

        let d9: FixedPointNumber<SIZE, PRECISION, T, C> = FromWithContext::from_ctx(0.0, ctx);
        let d8: FixedPointNumber<SIZE, PRECISION, T, C> = FromWithContext::from_ctx(0.015625, ctx);
        let d7: FixedPointNumber<SIZE, PRECISION, T, C> = FromWithContext::from_ctx(0.03125, ctx);
        let d6: FixedPointNumber<SIZE, PRECISION, T, C> = FromWithContext::from_ctx(0.125, ctx);
        let d5: FixedPointNumber<SIZE, PRECISION, T, C> = FromWithContext::from_ctx(0.25, ctx);

        let der = comp1.mux_number(&d8, &d9);
        let der = comp2.mux_number(&d7, &der);
        let der = comp3.mux_number(&d6, &der);
        let der = comp4.mux_number(&d5, &der);
        let der = comp5.mux_number(&d6, &der);
        let der = comp6.mux_number(&d7, &der);
        let der = comp7.mux_number(&d8, &der);
        let der = comp8.mux_number(&d9, &der);
    ...
    fn activate_multiple(
        ...
        let res = lst.apply_zip(&v8, |(v, v8)| v.is_greater(&f8).mux_number(&v8, &v9));
        let res = lst.apply_zip_2(&v7, &res, |(v, v7, res)| {
            v.is_greater(&f7).mux_number(&v7, &res)
        });
        let res = lst.apply_zip_2(&v6, &res, |(v, v6, res)| {
            v.is_greater(&f6).mux_number(&v6, &res)
        });
        let res = lst.apply_zip_2(&v5, &res, |(v, v5, res)| {
            v.is_greater(&f5).mux_number(&v5, &res)
        });
        let res = lst.apply_zip_2(&v4, &res, |(v, v4, res)| {
            v.is_greater(&f4).mux_number(&v4, &res)
        });
        let res = lst.apply_zip_2(&v3, &res, |(v, v3, res)| {
            v.is_greater(&f3).mux_number(&v3, &res)
        });
        let res = lst.apply_zip_2(&v2, &res, |(v, v2, res)| {
            v.is_greater(&f2).mux_number(&v2, &res)
        });
        let mut res = lst.apply_zip(&res, |(v, res)| v.is_greater(&f1).mux_number(&v1, &res));
    ...
    fn activate_and_derivative_multiple(
        ...
        */
        ...
        let res = comp1.apply_zip(&v8, |(v, v8)| v.clone().mux_number(&v8, &v9));
        let res = comp2.apply_zip_2(&v7, &res, |(v, v7, res)| v.clone().mux_number(&v7, &res));
        let res = comp3.apply_zip_2(&v6, &res, |(v, v6, res)| v.clone().mux_number(&v6, &res));
        let res = comp4.apply_zip_2(&v5, &res, |(v, v5, res)| v.clone().mux_number(&v5, &res));
        let res = comp5.apply_zip_2(&v4, &res, |(v, v4, res)| v.clone().mux_number(&v4, &res));
        let res = comp6.apply_zip_2(&v3, &res, |(v, v3, res)| v.clone().mux_number(&v3, &res));
        let res = comp7.apply_zip_2(&v2, &res, |(v, v2, res)| v.clone().mux_number(&v2, &res));
        let mut res = comp8.apply_zip(&res, |(v, res)| v.clone().mux_number(&v1, &res));
        ...
        let der = comp1.apply(|v| v.clone().mux_number(&d8, &d9));
        let der = comp2.apply_zip(&der, |(v, der)| v.clone().mux_number(&d7, &der));
        let der = comp3.apply_zip(&der, |(v, der)| v.clone().mux_number(&d6, &der));
        let der = comp4.apply_zip(&der, |(v, der)| v.clone().mux_number(&d5, &der));
        let der = comp5.apply_zip(&der, |(v, der)| v.clone().mux_number(&d6, &der));
        let der = comp6.apply_zip(&der, |(v, der)| v.clone().mux_number(&d7, &der));
        let der = comp7.apply_zip(&der, |(v, der)| v.clone().mux_number(&d8, &der));
        let mut der = comp8.apply_zip(&der, |(v, der)| v.clone().mux_number(&d9, &der));
...
impl<const SIZE: usize, const PRECISION: usize, T: BooleanType<C>, C, const N: usize>
    ActivationFn<FixedPointNumber<SIZE, PRECISION, T, C>, C, N> for SigmoidMyersMulAproxAF
where
    FixedPointNumber<SIZE, PRECISION, T, C>: FromWithContext<f32, C>,
{
    fn activate(
    ...
    fn activate_multiple(
        &self,
        _ctx: &Arc<C>,
        _lst: Gen1DArray<FixedPointNumber<SIZE, PRECISION, T, C>, C, N>,
    ) -> Gen1DArray<FixedPointNumber<SIZE, PRECISION, T, C>, C, N> {
        todo!()
    }
...
#[allow(deprecated)]
impl<const SIZE: usize, const PRECISION: usize, T: BooleanType<C>, C, const N: usize>
    ActivationFn<FixedPointNumber<SIZE, PRECISION, T, C>, C, N> for SigmoidAF
where
    FixedPointNumber<SIZE, PRECISION, T, C>: FromWithContext<f32, C>,
{
    fn activate(
    ...
    fn activate_and_derivative(
        ...
        let sig =
            <SigmoidAF as ActivationFn<FixedPointNumber<SIZE, PRECISION, T, C>, C, N>>::activate(
                &self, ctx, v,
            );
    ...
    fn activate_and_derivative_multiple(
        ...
        lst.apply_two(|v| <SigmoidAF as ActivationFn<FixedPointNumber<SIZE, PRECISION, T, C>, C, N>>::activate_and_derivative(self, ctx, v.clone()))
    }
}
...
mod test {
    ...
    fn test_real() {
        ...
        let test = |x: f32, _y: f32, _threathold: f32| {
            let v: FixedPointNumber<64, 32, bool, ()> = FromWithContext::from_ctx(x, &ctx);
            let res =
                <SigmoidAF as ActivationFn<FixedPointNumber<64, 32, bool, ()>, (), 1>>::activate(
                    &af, &ctx, v,
                );
